<template>
  <section>
    ...
    <el-table :data="pageInfo.rows" highlight-current-row v-loading="listLoading" @selection-change="selsChange"
      ...
      <el-table-column prop="entryTime" label="进入时间" width="240" sortable>
      </el-table-column>
      <el-table-column prop="leaveTime" label="离开时间" width="240" sortable>
      </el-table-column>
      <el-table-column prop="info" label="信息" width="250" sortable>
      </el-table-column>
    </el-table>
    ...
    <el-dialog title="新增/修改" :visible.sync="saveFormVisible" :close-on-click-modal="false">
      <el-form :model="saveForm" label-width="80px" :rules="saveFormRules" ref="addForm">
        <el-form-item label="姓名" style="width: 300px">
          <el-input type="text" v-model="saveForm.name"></el-input>
        </el-form-item>
        ...
        <el-form-item label="信息">
          <el-input type="text" v-model="saveForm.info"></el-input>
        </el-form-item>
      </el-form>
...
export default {
  ...
  methods: {
    ...
    search() {
      ...
      //调用查询
      this.getPersonEntryAndExitRecords();
    },
    ...
    //获取进出入人员记录
    getPersonEntryAndExitRecords() {
      this.$http.post("/personEntryAndExitRecords/page", this.query)
          .then(result => {
            result = result.data;
            if (result.success) {
    ...
    batchRemove: function () {
      ...
      this.$confirm('确认删除选中记录吗？', '提示', {
        ...
        // 调用批量删除接口
        this.$http.patch("/personEntryAndExitRecords", ids)
            .then(result => {
              result = result.data;
      ...
      this.$refs.addForm.validate((valid) => {
        if (valid) {
          this.$confirm('确认提交吗？', '提示', {}).then(() => {
            ...
            this.$http.post("/personEntryAndExitRecords/save", para)
                .then(result => {
                  result = result.data;
                  if (result.success) {
                    //从第一页开始展示
